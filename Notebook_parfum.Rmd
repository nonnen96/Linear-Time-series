...
```{r}
arima(times_series_diff,c(1,1,2))
```
...
```{r}
fit <- Arima(times_series_diff, order = c(1, 1, 2))
...
plot(forecast_values, main = "Confidence region at 95% for an ARIMA(1,1,2)", xlim=c(30,37), col="darkblue",lwd=2,  flwd=1,shadecols = rgb(0.8, 0.8, 0.8, 0.5), ylab = "Values of the series",xlab = "Time")  
```
