...
plot(time_series_clean, type = "l", col = "blue", lwd = 2, 
     xlab = "Date", ylab = "Values", 
     main = "Time Series Plot of values")
```
```{r}
decomposition <- decompose(time_series_clean, 'additive')
plot(decomposition)
```

```{r}
acf(time_series_clean)
pacf(time_series_clean)
```
...
```{r}
time_series_zoo_diff <- diff(time_series_clean)
...
```{r}
ts_test<-ts(time_series_zoo_diff, frequency=12)
decomposition <- decompose(ts_test, 'additive')
plot(decomposition)
```
...
```{r}
time_series_diff=ts(time_series_zoo_diff, frequency = 12)
...
```{r}
adf.test(time_series_diff)
pp.test(time_series_diff)
kpss.test(time_series_diff)
```
...
for (p in 0:2) {
  for (q in 0:1) {
    fit <- Arima(time_series_clean, order = c(p, 1, q))
...
```{r}
arima(time_series_clean,c(0,1,1))
```
...
```{r}
fit <- Arima(time_series_clean, order = c(0, 1, 1))
...
checkresiduals(fit)
res <- residuals(fit)
plot(res)             # Plot residuals over time
acf(res)              # ACF of residuals
qqnorm(res); qqline(res, col = "red")  # Q-Q plot for normality
shapiro.test(res)
forecast_values <- forecast(fit, h = 2)
plot(forecast_values, main = "Confidence region at 95% for an ARIMA(0,1,1)", xlim=c(30,37),ylim=c(90, 125), col="darkblue",lwd=2,  flwd=1,shadecols = rgb(0.8, 0.8, 0.8, 0.5), ylab = "Values of the series",xlab = "Time")  
```

We can now graphically represent the confidence region for α = 95%. The light gray area shows the 95% confidence interval. The dark blue points correspond to the prediction.
