...
```
```{r}
decomposition <- decompose(time_series, 'additive')
plot(decomposition)
```
...
```{r}
time_series_zoo_diff <- diff(time_series)
...
pp.test(times_series_diff)
kpss.test(times_series_diff)
```
...
```{r}
arima(times_series_diff,c(3,1,2))
```
...
```{r}
fit <- Arima(times_series_diff, order = c(3, 1, 2))
...
plot(forecast_values, main = "Confidence region at 95% for an ARIMA(2,1,9)", xlim=c(32,37),ylim=c(-0.2, 0.2), col="darkblue",lwd=2,  flwd=1,shadecols = rgb(0.8, 0.8, 0.8, 0.5), ylab = "Values of the series",xlab = "Time")  
```
